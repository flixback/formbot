  ...
  script.scripts.run({
    ...
    if (resp.error) {
      ...
      for(i=0; i< nb_items; i++){
        ...
        if(myForm.items[index].type == "SECTION_HEADER"){
          
          bot.add('/'+i, function (session) {
            session.send(myForm.items[index].title);
            index++;
            session.endDialog();
          });
        }			
          ...
          bot.add('/'+i, [
            ...
            var item = myForm.items[index];
            askItem(session,item);
          },
...
function askItem(session,item) {
  
  switch (item.type)  {
      
      
    case "CHECKBOX":
      builder.Prompts.choice(session, item.title, item.choices);
      
      break;
      
    case "DATE":
      builder.Prompts.time(session, item.title);
    ...
    case "TEXT":
      builder.Prompts.text(session,item.title );
    ...
    case "SCALE":
      builder.Prompts.number(session,"On a scale from "+item.lowerBound+" ("+item.leftLabel+") to "+item.upperBound+" ("+item.rightLabel+"), "+item.title ); 
      
      break;
      
    default:
      Logger.log("NOT SUPPORTED AT THE MOMENT");
      break;
  }
}
